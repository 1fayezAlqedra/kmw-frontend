<template>
  <div class="w-full mx-auto animate-fade-in box-border bg-[#F7F4F0] min-h-screen p-4 sm:p-6 md:p-8" dir="ltr">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h4 class="text-lg md:text-xl font-black text-slate-900 tracking-wide uppercase">EDIT PROJECT CATEGORY</h4>
        <p class="text-xs text-slate-400 font-bold mt-1">Update existing marble project category details and cover image</p>
      </div>
    ...
    <!-- Loading State -->
    <div v-if="isLoading" class="bg-white rounded-2xl border border-[#EAE3DA] p-12 text-center shadow-xs">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-[#8B5E1A] border-t-transparent"></div>
      <p class="mt-3 text-xs font-bold text-slate-500 uppercase tracking-wider">LOADING CATEGORY DATA...</p>
    </div>

    <!-- Form Container -->
    <div v-else class="bg-white rounded-2xl border border-[#EAE3DA] shadow-[0_4px_20px_-4px_rgba(139,92,26,0.05)] w-full overflow-hidden">
      <form @submit.prevent="handleSubmit" class="p-5 md:p-8 space-y-6 md:space-y-8" enctype="multipart/form-data">
        ...
        <div class="bg-[#F7F4F0]/40 border border-[#EAE3DA] rounded-2xl p-4 sm:p-6 md:p-8 space-y-6" dir="ltr">
          <div>
            ...
            <p class="text-[11px] text-slate-400 font-bold mt-0.5">Upload a new image to replace the current category cover</p>
          </div>
          ...
          >
            ...
            <p class="text-xs font-black text-slate-700 uppercase tracking-wider">
              DRAG & DROP NEW IMAGE OR <span class="text-[#8B5E1A] underline cursor-pointer">BROWSE</span>
            </p>
            <p class="text-[10px] text-slate-400 font-bold mt-1">Leave empty to keep existing image</p>
          </div>
          ...
          <div v-if="imagePreview" class="relative w-40 aspect-[4/3] rounded-xl border border-[#EAE3DA] bg-white p-1.5 shadow-xs group overflow-hidden">
            <img :src="imagePreview" class="w-full h-full object-cover rounded-lg" alt="Category Preview" />
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-[#EAE3DA]/60" dir="ltr">
          ...
          >
            {{ isSubmitting ? 'SAVING CHANGES...' : 'UPDATE CATEGORY' }}
          </button>
        </div>
...
onMounted(async () => {
  try {
    const response = await apiClient.get(`project-categories/${categoryId}`)
    const category = response.data?.data || response.data

    if (category) {
      form.value.name_en = category.name_en || ''
      form.value.name_ar = category.name_ar || ''
      form.value.description_en = category.description_en || ''
      form.value.description_ar = category.description_ar || ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error.response && error.response.status === 422) {
      ...
      errorMessage.value = error.response?.data?.message || 'Failed to update category. Please try again.'
    }
  } finally {
