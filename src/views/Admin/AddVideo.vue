<template>
  <div class="w-full max-w-5xl mx-auto animate-fade-in box-border bg-[#F7F4F0] min-h-screen p-4 sm:p-6 md:p-8" dir="ltr">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h3 class="text-xl md:text-2xl font-black text-[#0A1A2F] tracking-tight uppercase">ADD NEW VIDEO</h3>
        <p class="text-xs text-[#5C728D] font-bold mt-1">Add a main stone video classification (e.g., Marble, Granite, Quartz)</p>
      </div>
      <RouterLink
        to="/admin/videos"
      ...
      <form @submit.prevent="handleSubmit" class="p-6 md:p-10 space-y-6 md:space-y-8">
        ...
        <div class="flex flex-col space-y-2">
          ...
          <div class="relative">
            <input
              ...
              placeholder="Paste link from YouTube, Instagram, Facebook, or TikTok..."
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-[#EAE3DA]/60">
          ...
          <button
            ...
            class="w-full sm:w-auto px-8 py-3 bg-[#A1461D] hover:bg-amber-950 text-white text-xs font-black rounded-xl transition-all duration-300 uppercase tracking-widest text-center cursor-pointer disabled:opacity-50 shadow-sm flex items-center justify-center gap-2"
          >
            <span>{{ isSubmitting ? 'Publishing...' : 'Publish Video Asset' }}</span>
          </button>
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    const payload = {
      ...
      youtube_url: form.value.video_url, // إرسال كلا المفتاحين للتوافق التام مع الباك إيند
      platform: detectedPlatform.value || 'unknown',
    ...
    if (error.response?.status === 422) {
      ...
      alert(`Validation Error: ${firstError || 'Check your fields.'}`)
    } else if (error.response?.status === 401) {
      alert('Session expired. Please log in again.')
      router.push('/login')
    } else {
      alert('Failed to publish video. Please check your network or input.')
    }
  } finally {
