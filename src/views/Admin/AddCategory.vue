<template>
  <div class="w-full max-w-4xl mx-auto p-4 sm:p-0 animate-fade-in box-border overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#EAE3DA] pb-6 mb-8">
      <div>
        <h3 class="text-xl md:text-2xl font-black text-slate-900 tracking-wide uppercase">Create New Category</h3>
        <p class="text-xs text-slate-400 font-bold mt-1">Add a main stone classification (e.g., Marble, Granite, Quartz)</p>
      </div>
      <RouterLink
        to="/admin/dashboard"
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex flex-col space-y-2">
          <label class="text-xs font-black text-slate-400 uppercase tracking-wider">
            URL Slug <span class="text-[10px] text-amber-700/70 font-black font-mono">(Auto-generated for SEO)</span>
          </label>
          <div class="relative flex items-center w-full">
            ...
            <input
              ...
              required
              placeholder="granite"
              class="w-full pl-24 pr-4 py-3.5 bg-[#F7F4F0]/30 border border-[#EAE3DA] rounded-xl text-amber-900 font-mono text-xs shadow-sm outline-none cursor-not-allowed truncate"
              readonly
            />
          </div>
        </div>
        ...
        <div class="flex flex-col space-y-2">
          ...
          <div class="border-2 border-dashed border-[#EAE3DA] hover:border-amber-950/40 rounded-2xl p-5 sm:p-6 bg-[#F7F4F0]/20 transition-all duration-300 flex flex-col items-center justify-center text-center group cursor-pointer relative min-h-[180px]">
            ...
            <input type="file" @change="handleFileUpload" accept="image/*" required class="absolute inset-0 opacity-0 cursor-pointer" />
          </div>
        </div>
